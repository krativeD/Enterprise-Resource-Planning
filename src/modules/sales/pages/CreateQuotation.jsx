...
function QuotationTemplate({ quotation, items }) {
  ...
  return (
    <div style={{ width: '210mm', minHeight: '297mm', padding: '15mm 20mm', backgroundColor: 'white', fontFamily: 'Arial, sans-serif', color: '#1e293b', boxSizing: 'border-box' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px', borderBottom: '3px solid #2563eb', paddingBottom: '15px' }}>
        ...
        <div style={{ textAlign: 'right' }}>
          <h2 style={{ fontSize: '26px', fontWeight: 'bold', color: '#1e3a5f', margin: '0' }}>QUOTATION</h2>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '20px' }}>
        <thead><tr style={{ backgroundColor: '#1e40af', color: 'white' }}><th style={{ padding: '8px 10px', textAlign: 'left', fontSize: '10px', fontWeight: 'bold' }}>#</th><th style={{ padding: '8px 10px', textAlign: 'left', fontSize: '10px', fontWeight: 'bold' }}>Description</th><th style={{ padding: '8px 10px', textAlign: 'center', fontSize: '10px', fontWeight: 'bold' }}>Qty</th><th style={{ padding: '8px 10px', textAlign: 'right', fontSize: '10px', fontWeight: 'bold' }}>Unit Price</th><th style={{ padding: '8px 10px', textAlign: 'right', fontSize: '10px', fontWeight: 'bold' }}>Total</th></tr></thead>
        <tbody>{items.map((item, i) => (<tr key={i} style={{ borderBottom: '1px solid #e2e8f0' }}><td style={{ padding: '8px 10px', fontSize: '10px', color: '#64748b' }}>{i + 1}</td><td style={{ padding: '8px 10px', fontSize: '10px', color: '#1e293b', fontWeight: '500' }}>{item.description || 'Service'}</td><td style={{ padding: '8px 10px', fontSize: '10px', color: '#1e293b', textAlign: 'center' }}>{item.quantity || 1}</td><td style={{ padding: '8px 10px', fontSize: '10px', color: '#1e293b', textAlign: 'right' }}>{formatCurrency(item.unit_price || 0)}</td><td style={{ padding: '8px 10px', fontSize: '10px', color: '#1e293b', textAlign: 'right', fontWeight: '600' }}>{formatCurrency((item.quantity || 1) * (item.unit_price || 0))}</td></tr>))}</tbody>
      </table>
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '20px' }}>
        <div style={{ width: '260px', border: '1px solid #e2e8f0', borderRadius: '5px', overflow: 'hidden' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 14px', borderBottom: '1px solid #e2e8f0', fontSize: '10px', backgroundColor: '#f8fafc' }}><span style={{ color: '#64748b' }}>Subtotal:</span><span style={{ color: '#1e293b', fontWeight: '600' }}>{formatCurrency(subtotal)}</span></div>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 14px', borderBottom: '1px solid #e2e8f0', fontSize: '10px', backgroundColor: '#f8fafc' }}><span style={{ color: '#64748b' }}>VAT (15%):</span><span style={{ color: '#1e293b' }}>{formatCurrency(taxAmount)}</span></div>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 14px', fontSize: '14px', fontWeight: 'bold', backgroundColor: '#eff6ff' }}><span style={{ color: '#1e40af' }}>TOTAL:</span><span style={{ color: '#1e40af', fontSize: '16px' }}>{formatCurrency(total)}</span></div>
        </div>
      </div>
...
export default function CreateQuotation() {
  ...
  const [quotationData, setQuotationData] = useState({
    ...
    payment_terms: '30 Days', tax_rate: 15, discount_type: 'none', discount_value: 0,
    notes: '', terms_and_conditions: '', status: 'draft'
  })

  const [items, setItems] = useState([{ description: '', quantity: 1, unit: 'per_hour', unit_price: 0, tax_percent: 15, discount_percent: 0 }])

  useEffect(() => { fetchClients({ status: 'active' }); fetchProductsServices() }, [])

  const handleClientSelect = (clientId) => {
    const client = clients.find(c => c.id === clientId)
    if (client) {
      setQuotationData({...quotationData, client_id: client.id, client_name: client.company_name, 
  ...
  const handleSave = async (status = 'draft') => {
    ...
    if (result.error) { toast.error('Failed to save quotation: ' + result.error.message); return }
  ...
  const downloadPDF = () => {
    ...
    html2pdf().set({ margin: [5, 5, 5, 5], filename: 'Quotation.pdf', image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2, useCORS: true }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' }, pagebreak: { mode: 'avoid-all' } }).from(element).save()
  }
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-16">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex gap-3">
            <button onClick={downloadPDF} className="neu-raised neu-btn px-4 py-2 rounded-xl flex items-center gap-2 bg-blue-600 text-white hover:bg-blue-700"><Download className="w-4 h-4" />Download PDF</button>
            <button onClick={() => handleSave('draft')} className="neu-raised neu-btn px-4 py-2 rounded-xl flex items-center gap-2 bg-slate-600 text-white"><Save className="w-4 h-4" />Save Draft</button>
            <button onClick={() => handleSave('sent')} className="neu-raised neu-btn px-4 py-2 rounded-xl flex items-center gap-2 bg-emerald-600 text-white"><Send className="w-4 h-4" />Save & Send</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          <div className="space-y-6">
            ...
            <div className="neu-raised rounded-3xl p-6">
              <div className="flex items-center justify-between mb-4"><h2 className="text-lg font-semibold">Items</h2><button onClick={addItem} className="text-emerald-600 flex items-center gap-1 text-sm"><Plus className="w-4 h-4" />Add</button></div>
              <div className="space-y-4">
                {items.map((item, index) => (
                  <div key={index} className="p-4 rounded-xl bg-slate-50 dark:bg-slate-700/30 space-y-3">
                    <div className="flex justify-between"><span className="text-sm">Item {index + 1}</span><button onClick={() => removeItem(index)} className="text-red-500"><Trash2 className="w-4 h-4" /></button></div>
                    <select onChange={(e) => handleProductSelect(index, e.target.value)} className="w-full p-2 neu-inset rounded-lg text-sm"><option value="">Select Product</option>{productsServices.map(ps => <option key={ps.id} value={ps.id}>{ps.name}</option>)}</select>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      ...
                      <select value={item.unit} onChange={e => updateItem(index, 'unit', e.target.value)} className="p-2 neu-inset rounded-lg text-sm"><option value="each">Each</option><option value="per_hour">Per Hour</option><option value="per_sqm">Per m²</option><option value="per_month">Per Month</option></select>
          ...
          <div className="lg:sticky lg:top-24 h-fit">
            <div className="neu-raised rounded-3xl p-4">
              ...
              <div className="bg-white rounded-xl overflow-hidden border" style={{ maxHeight: '500px', overflow: 'auto' }}>
                <div style={{ transform: 'scale(0.5)', transformOrigin: 'top left', width: '200%' }}>
                  <div id="quotation-pdf-download"><QuotationTemplate quotation={quotationData} items={items} /></div>
                </div>
              </div>
            </div>
            <div className="neu-raised rounded-3xl p-6 mt-4">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between"><span>VAT (15%):</span><span>{formatCurrency(totals.taxAmount)}</span></div>
